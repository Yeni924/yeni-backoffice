...
주문 관리는 결제 흐름의 출발점으로 구성했습니다. 사용자가 결제금액을 직접 입력하는 방식이 아니라, 주문 상품 상세를 기준으로 상품합계, 배송비, 할인금액, 최종 결제금액을 서버에서 계산합니다.

## 권장 시연 순서

포트폴리오 데모는 아래 순서로 보면 주문에서 결제, 매출 원장, 후속처리, 정산까지 한 흐름으로 확인할 수 있습니다.

1. 주문 관리(`/admin/commerce/orders`)에서 Mock 주문을 생성하거나 주문 상품을 직접 입력합니다.
2. 주문 목록에서 결제 승인을 실행해 기존 PG 승인 로직으로 연결되는지 확인합니다.
3. PG 운영(`/admin/payment-operations`)에서 결제 거래, 승인/취소/결과불명/망취소, 외부전송, 알림톡, RecoveryTask 흐름을 확인합니다.
4. 매출 원장(`/admin/payment-operations/sales-ledger`)에서 SALE 결제매출과 CANCEL 취소매출이 원매출 수정 없이 별도 거래로 남는지 확인합니다.
5. 정산 관리(`/admin/payment-operations/settlements`)에서 오늘 정산 배치를 실행하고 매출 원장 기준 정산 초안이 생성되는지 확인합니다.

핵심 흐름은 `주문 관리 -> PG 운영 -> 매출 원장 -> 후속처리/복구 -> 정산 관리`입니다. 면접 시에는 먼저 주문 화면에서 시작해 실제 커머스 백오피스처럼 결제 이후 데이터가 어디로 이어지는지 설명하는 방식이 가장 자연스럽습니다.

현재 구현 범위는 상품 마스터, 옵션/재고, 장바구니, 쿠폰/프로모션, 배송/반품까지 확장하지 않고 주문 snapshot과 기존 결제 운영 흐름 연결에 집중합니다.

```text
주문 상품 상세 입력
  -> 상품금액 = 단가 * 수량
  -> 상품합계 + 배송비 - 할인금액 = 최종 결제금액
  -> 기존 PG 승인 서비스 재사용
...
이번 보강으로 `/admin/commerce/orders` 화면에서 주문 상품 행을 추가/삭제하고, 최종 결제금액을 확인한 뒤 주문 결제 승인을 실행할 수 있습니다.
